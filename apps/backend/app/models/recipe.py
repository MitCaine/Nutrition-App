...
from sqlalchemy import Boolean, CheckConstraint, DateTime, ForeignKey, Integer, Numeric, Text, UniqueConstraint, false, func
...
class Recipe(Base):
    ...
    __table_args__ = (
        CheckConstraint("serving_count_yield IS NULL OR serving_count_yield > 0", name="ck_recipes_serving_count_positive"),
        CheckConstraint(
    ...
    needs_republish: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=false())
...
class RecipeIngredient(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("resolved_gram_amount IS NULL OR resolved_gram_amount > 0", name="ck_recipe_ingredients_grams_positive"),
    )

    id: Mapped[UUID] = mapped_column(GUID(), primary_key=True)
    recipe_id: Mapped[UUID] = mapped_column(GUID(), ForeignKey("recipes.id", ondelete="CASCADE"), nullable=False)
