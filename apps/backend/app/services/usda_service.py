...
class UsdaService:
    ...
    def search(
        ...
        payload = self.client.search_foods(
            query=query, page_size=page_size, page_number=page_number
        )
        return map_search_response(
            payload, query=query, page_size=page_size, page_number=page_number
        )
    ...
    def import_food(self, user_id: UUID, fdc_id: int) -> tuple[FoodItem, bool]:
        """Own one import transaction and its source-identity race reconciliation."""
        try:
            ...
            self._after_import_creation(created)
            self.db.commit()
    ...
    def _after_import_creation(self, _food: FoodItem) -> None:
        """Test seam after the imported Food graph is flushed."""
...
def _is_source_identity_conflict(exc: IntegrityError) -> bool:
    ...
    if constraint_name == "ix_food_items_active_source_identity":
        return True
    if getattr(orig, "sqlstate", None) == "23505" and "ix_food_items_active_source_identity" in str(
        orig
    ):
        return True
