...
class Settings(BaseSettings):
    ...
    def validate_release_configuration(self) -> Settings:
        try:
            ...
            raise ValueError("NUTRITION_DATABASE_URL must be a valid SQLAlchemy database URL") from exc
        ...
        if self.deployment_mode is DeploymentMode.PRIVATE_SINGLE_USER:
            if self.private_auth_secret is None or len(self.private_auth_secret.get_secret_value()) < 32:
                raise ValueError(
