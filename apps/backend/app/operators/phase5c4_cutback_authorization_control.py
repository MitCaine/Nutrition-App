"""Narrow control client for Phase 5C4.8 cutback authorization."""
...
_PRIMARY_REASONS = {
    "cutback_authorization_binding_stale": "cutback_authorization_binding_stale",
    "cutback_authorization_conflict": "cutback_authorization_conflict",
    "cutback_authorization_invalid": "cutback_authorization_invalid",
    "cutback_authorization_key_conflict": "cutback_authorization_key_conflict",
    "cutback_authorization_key_invalid": "cutback_authorization_key_invalid",
    "cutback_authorization_key_unknown": "cutback_authorization_key_unknown",
    "cutback_authorization_key_untrusted": "cutback_authorization_key_untrusted",
    "cutback_authorization_revoked": "cutback_authorization_revoked",
    "cutback_authorization_serialization_race": "cutback_authorization_retry",
    "cutback_authorization_time_invalid": "cutback_authorization_time_invalid",
    "phase5c4_control_unauthorized": "cutback_authorization_unauthorized",
}


class Phase5C4CutbackAuthorizationControlError(RuntimeError):
...
def _database_error(
    ...
    reason = next(
        ...
        "cutback_authorization_internal_failure",
    )
    retryable = sqlstate in _TRANSIENT_SQLSTATES or sqlstate.startswith("08")
    if retryable:
        reason = "cutback_authorization_retry"
    elif sqlstate == "42501":
        reason = "cutback_authorization_unauthorized"
    return Phase5C4CutbackAuthorizationControlError(
        reason,
        retryable=retryable,
    )
...
def _run(
    ...
    *,
    ...
    try:
    ...
    except Phase5C4CutbackAuthorizationControlError:
        raise
        ...
        engine.dispose()
    raise Phase5C4CutbackAuthorizationControlError(
        "cutback_authorization_retry",
        retryable=True,
    )


def read_trusted_cutback_public_key(
    database_url: str,
    key_id: str,
) -> dict[str, Any]:
    def operation(connection):
        row = (
            connection.execute(
                text(
                    ...
                    SELECT *
                    FROM phase5c4_api.
                        read_cutback_authorization_key_v1(:key_id)
                    """
                ),
                {"key_id": key_id},
            )
        ...
        if row is None:
            raise Phase5C4CutbackAuthorizationControlError("cutback_authorization_key_unknown")
        return dict(row)

    return _run(database_url, operation, serializable=False, retries=1)


def _require_trusted_key(key: dict[str, Any]) -> None:
    if (
        key["revoked_at"] is not None
        or key["authority_time"] < key["valid_from"]
        or key["authority_time"] >= key["valid_until"]
    ):
        raise Phase5C4CutbackAuthorizationControlError("cutback_authorization_key_untrusted")


def verify_cutback_with_trust_store(
    database_url: str,
    canonical_bytes: bytes,
) -> dict[str, Any]:
    envelope = parse_cutback_authorization_envelope(canonical_bytes)
    key_id = str(envelope["signed"]["key_id"])
    key = read_trusted_cutback_public_key(database_url, key_id)
    _require_trusted_key(key)
    verified = verify_cutback_authorization(
        canonical_bytes,
...
def verify_and_admit_cutback_authorization(
    database_url: str,
    canonical_bytes: bytes,
) -> dict[str, Any]:
    envelope = parse_cutback_authorization_envelope(canonical_bytes)
        ...
        admitted = (
            connection.execute(
                text(
                    """
                    SELECT *
                    FROM phase5c4_api.admit_cutback_authorization_v1(
                        :canonical_bytes
                    )
                    """
                ),
...
def bootstrap_cutback_authorization_key(
    database_url: str,
    *,
    ...
    def operation(connection):
        row = (
            connection.execute(
                text(
                    ...
                    SELECT *
                    FROM phase5c4_api.
                        bootstrap_cutback_authorization_key_v1(
        ...
        if str(row["key_id"]) != expected_key_id:
            raise Phase5C4CutbackAuthorizationControlError("cutback_authorization_key_invalid")
        return {"key_id": expected_key_id, "result": str(row["result"])}

    return _run(database_url, operation)


def revoke_cutback_authorization_key(
    database_url: str,
    *,
    key_id: str,
    reason: str,
        ...
        row = (
            connection.execute(
                text(
                    ...
                    SELECT *
                    FROM phase5c4_api.
                        revoke_cutback_authorization_key_v1(
...
def revoke_cutback_authorization(
    database_url: str,
    *,
    authorization_id: str,
    reason: str,
        ...
        row = (
            connection.execute(
                text(
                    """
                    SELECT *
                    FROM phase5c4_api.revoke_cutback_authorization_v1(
...
def read_cutback_authorization_status(
    database_url: str,
    authorization_id: str,
) -> dict[str, Any] | None:
    canonical_id = str(UUID(authorization_id))
        ...
        row = (
            connection.execute(
                text(
                    """
                    SELECT *
                    FROM phase5c4_api.read_cutback_authorization_v1(
                        CAST(:authorization_id AS uuid)
                    )
                    """
                ),
