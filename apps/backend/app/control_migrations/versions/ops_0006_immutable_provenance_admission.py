"""Install immutable-provenance qualification admission.

Revision ID: ops_0006_immutable_provenance
Revises: ops_0005_resource_membership
Create Date: 2026-07-21

The ops-0004 and ops-0005 definitions and evidence remain immutable.  This
revision adds a versioned 0020 receipt and the next exact control catalog.
"""
...
down_revision = PREVIOUS_CONTROL_SCHEMA_REVISION
branch_labels = None
depends_on = None
...
def _require_postgresql() -> None:
    if op.get_bind().dialect.name != "postgresql":
        raise RuntimeError("Immutable provenance control admission is PostgreSQL-only")
...
def _verify_ops5_baseline() -> None:
    op.execute(
        f"""
        DO $block$
        ...
        BEGIN
            IF (SELECT version_num
                FROM phase5c4_control.phase5c4_alembic_version) <>
                    '{PREVIOUS_CONTROL_SCHEMA_REVISION}' THEN
                RAISE EXCEPTION 'immutable_provenance_control_baseline_invalid'
                    USING ERRCODE = 'P5C44';
            END IF;
            WITH actual AS (
            ...
            FROM phase5c4_control.phase5c4_qualification_v3_catalog_manifest expected
...
def _install_storage() -> None:
    op.execute(
        f"""
        CREATE TABLE phase5c4_control.phase5c4_immutable_provenance_admissions (
            ...
            contract_version phase5c4_control.bounded_name NOT NULL
                CHECK (contract_version = '{IMMUTABLE_PROVENANCE_QUALIFICATION_VERSION}'),
            ...
            immutable_manifest_digest phase5c4_control.sha256_digest NOT NULL,
        ...
        CREATE TRIGGER phase5c4_immutable_provenance_admission_row
            BEFORE UPDATE OR DELETE
            ON phase5c4_control.phase5c4_immutable_provenance_admissions
            FOR EACH ROW EXECUTE FUNCTION
                phase5c4_control.phase5c4_reject_immutable_change();
        CREATE TRIGGER phase5c4_immutable_provenance_admission_truncate
            BEFORE TRUNCATE
            ON phase5c4_control.phase5c4_immutable_provenance_admissions
            FOR EACH STATEMENT EXECUTE FUNCTION
                phase5c4_control.phase5c4_reject_immutable_change();

        CREATE TABLE phase5c4_control.phase5c4_qualification_v4_catalog_manifest (
        ...
        CREATE TRIGGER phase5c4_immutable_v4_catalog_row
            BEFORE UPDATE OR DELETE
            ON phase5c4_control.phase5c4_qualification_v4_catalog_manifest
            FOR EACH ROW EXECUTE FUNCTION
                phase5c4_control.phase5c4_reject_immutable_change();
        CREATE TRIGGER phase5c4_immutable_v4_catalog_truncate
            BEFORE TRUNCATE
            ON phase5c4_control.phase5c4_qualification_v4_catalog_manifest
            FOR EACH STATEMENT EXECUTE FUNCTION
                phase5c4_control.phase5c4_reject_immutable_change();
...
def _install_admission_api() -> None:
    ...
    keys_sql = "ARRAY[" + ",".join(_sql_literal(key) for key in expected_keys) + "]::text[]"
    ...
    immutable_json = canonical_json(immutable_manifest)
    runtime_json = canonical_json(runtime_privileges)
    constraint_digest = canonical_digest(
        {
            "constraint_manifest_version": CONSTRAINT_MANIFEST_VERSION,
            "constraints": constraints,
        }
    )
    immutable_digest = canonical_digest(immutable_manifest)
    runtime_digest = canonical_digest(runtime_privileges)
    op.get_bind().exec_driver_sql(
        f"""
        CREATE FUNCTION phase5c4_api.admit_immutable_provenance_v1(
        ...
        BEGIN
            ...
            IF p_canonical_bytes IS NULL
               OR octet_length(p_canonical_bytes) NOT BETWEEN 2 AND 16777216 THEN
                RAISE EXCEPTION 'immutable_provenance_admission_invalid'
                    USING ERRCODE = '22023';
            END IF;
            BEGIN
            ...
            IF observed_keys IS DISTINCT FROM {keys_sql}
               ...
               OR document->>'constraint_manifest_version' <>
                    '{CONSTRAINT_MANIFEST_VERSION}'
               OR document->>'immutable_provenance_manifest_version' <>
                    '{IMMUTABLE_PROVENANCE_MANIFEST_VERSION}'
               ...
               OR document->'resource_membership_integrity_valid' <> 'true'::jsonb
               OR document->'immutable_provenance_integrity_valid' <> 'true'::jsonb
               OR document->'constraints' IS DISTINCT FROM
                    {_sql_literal(constraints_json)}::jsonb
               OR document->'immutable_provenance_manifest' IS DISTINCT FROM
                    {_sql_literal(immutable_json)}::jsonb
               OR document->'runtime_privileges' IS DISTINCT FROM
                    {_sql_literal(runtime_json)}::jsonb
               OR document->>'constraint_manifest_digest' <> '{constraint_digest}'
               OR document->>'immutable_provenance_manifest_digest' <>
                    '{immutable_digest}'
               OR document->>'runtime_privilege_digest' <> '{runtime_digest}' THEN
                RAISE EXCEPTION 'immutable_provenance_admission_invalid'
                    USING ERRCODE = '22023';
            END IF;
            ...
            IF document->>'qualification_digest' <> digest_value
               ...
               OR document->>'immutable_provenance_manifest_digest' <>
                    phase5c4_control.phase5c4_canonical_sha256(
                        document->'immutable_provenance_manifest'
                    )
            ...
            FROM phase5c4_control.phase5c4_immutable_provenance_admissions AS admission
            WHERE admission.qualification_digest = digest_value;
            IF stored_bytes IS NOT NULL THEN
                IF stored_bytes <> p_canonical_bytes THEN
                    RAISE EXCEPTION 'immutable_provenance_admission_conflict'
                        USING ERRCODE = 'P5C44';
                END IF;
            ...
            INSERT INTO phase5c4_control.phase5c4_immutable_provenance_admissions(
                qualification_digest, contract_version, schema_revision,
                constraint_manifest_digest, immutable_manifest_digest,
                runtime_privilege_digest, preflight_report_digest,
                ...
                document->>'immutable_provenance_manifest_digest',
                document->>'runtime_privilege_digest',
...
def _install_qualification_v4() -> None:
    op.execute(
        f"""
        CREATE FUNCTION phase5c4_api.qualify_control_plane_v4()
        RETURNS TABLE(
            control_admission_version text,
            migration_head text,
            event_chain_failures bigint,
            direct_public_table_grants bigint,
            immutable_provenance_admission_count bigint,
            qualified boolean
        )
        LANGUAGE plpgsql
        ...
        BEGIN
            ...
            FROM phase5c4_control.phase5c4_immutable_provenance_admissions;
            RETURN QUERY SELECT
                '{IMMUTABLE_PROVENANCE_CONTROL_ADMISSION_VERSION}'::text,
                head,
                chain_failures,
                public_grants,
                admission_count,
...
def _install_privileges_and_manifest() -> None:
    op.execute(
        ...
        REVOKE ALL ON TABLE
            ...
            phase5c4_control.phase5c4_qualification_v4_catalog_manifest
        ...
        REVOKE ALL ON FUNCTION
            phase5c4_api.admit_immutable_provenance_v1(bytea),
            phase5c4_api.qualify_control_plane_v4()
            FROM PUBLIC;
        ...
        INSERT INTO phase5c4_control.phase5c4_qualification_v4_catalog_manifest(
...
def upgrade() -> None:
    _require_postgresql()
    _verify_ops5_baseline()
    _install_storage()
    _install_admission_api()
    _install_qualification_v4()
...
def downgrade() -> None:
    ...
    if admissions:
        raise RuntimeError("Immutable provenance control admission is forward-only after use")
    op.execute(
        """
        DROP FUNCTION phase5c4_api.qualify_control_plane_v4();
        DROP FUNCTION phase5c4_api.admit_immutable_provenance_v1(bytea);
        DROP TRIGGER phase5c4_immutable_v4_catalog_truncate
            ON phase5c4_control.phase5c4_qualification_v4_catalog_manifest;
        DROP TRIGGER phase5c4_immutable_v4_catalog_row
            ON phase5c4_control.phase5c4_qualification_v4_catalog_manifest;
        DROP TABLE phase5c4_control.phase5c4_qualification_v4_catalog_manifest;
        DROP TRIGGER phase5c4_immutable_provenance_admission_truncate
            ON phase5c4_control.phase5c4_immutable_provenance_admissions;
        DROP TRIGGER phase5c4_immutable_provenance_admission_row
            ON phase5c4_control.phase5c4_immutable_provenance_admissions;
        DROP TABLE phase5c4_control.phase5c4_immutable_provenance_admissions;
        """
    )
