...
class TargetDatabase:
    ...
    def engine_authenticated_as(
            self,
            role: str,
            *,
            password: str | None = None,
    ) -> Engine:
        url = make_url(self.admin_url)
        return create_engine(
            url.set(
                username=role,
                password=password if password is not None else role,
            ),
            poolclass=NullPool,
            hide_parameters=True,
        )
    ...
    @contextmanager
    def connect_authenticated_as(
            self,
            role: str,
            *,
            password: str | None = None,
    ) -> Generator[Connection, None, None]:
        engine = self.engine_authenticated_as(role, password=password)
        try:
    ...
    def connect_as(self, role: str) -> Generator[Connection, None, None]:
        """Execute under SESSION AUTHORIZATION using the bootstrap login."""
        engine = self.engine()
        try:
            with engine.connect() as connection:
