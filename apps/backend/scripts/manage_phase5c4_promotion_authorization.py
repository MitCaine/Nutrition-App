"""Export, assemble, verify, and admit Phase 5C4.7a promotion authority."""
...
from app.operators.phase5c4_promotion_authorization_control import (
    ...
    revoke_promotion_authorization,
    revoke_promotion_authorization_key,
    verify_and_admit_promotion_authorization,
    verify_promotion_with_trust_store,
)


VERIFIER_URL_ENV = "NUTRITION_PHASE5C4_PROMOTION_AUTHORIZATION_VERIFIER_DATABASE_URL"
MIGRATOR_URL_ENV = "NUTRITION_CONTROL_MIGRATION_DATABASE_URL"


def _read_stable_file(path: Path, *, maximum_bytes: int = AUTHORIZATION_MAXIMUM_BYTES) -> bytes:
    flags = os.O_RDONLY
    ...
    try:
        ...
        raise Phase5C4AuthorizationError("promotion_authorization_file_invalid") from None
    try:
        ...
        if (
            ...
            raise Phase5C4AuthorizationError("promotion_authorization_file_invalid")
        chunks: list[bytes] = []
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Manage public Phase 5C4.7a promotion authorization material. "
            "This command cannot sign, route, activate, or open a target."
        )
    )
    ...
    assemble.add_argument("--promotion-authorization-out", type=Path, required=True)

    verify = commands.add_parser("verify")
    verify.add_argument("--promotion-authorization", type=Path, required=True)

    admit = commands.add_parser("admit")
    admit.add_argument("--promotion-authorization", type=Path, required=True)
...
def _required_url(name: str) -> str:
    ...
    if not value:
        raise Phase5C4PromotionAuthorizationControlError(
            "promotion_authorization_database_unavailable",
            retryable=True,
        )
    return value
...
def _parse_time(value: str) -> datetime:
    try:
        ...
        raise Phase5C4AuthorizationError("promotion_authorization_time_invalid") from None
    if canonical_timestamp(parsed) != value:
        raise Phase5C4AuthorizationError("promotion_authorization_time_invalid")
    return parsed
...
def _export(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        payload = parse_canonical_json(document, max_bytes=AUTHORIZATION_MAXIMUM_BYTES)
    except Exception:
        raise Phase5C4AuthorizationError("promotion_authorization_invalid") from None
    statement = build_promotion_signed_statement(payload, key_id=arguments.key_id)
...
def _assemble(arguments: argparse.Namespace) -> dict[str, Any]:
    statement = parse_promotion_signed_statement(_read_stable_file(arguments.statement))
    signature = _read_stable_file(arguments.signature_file, maximum_bytes=64)
    if len(signature) != 64:
        raise Phase5C4AuthorizationError("promotion_authorization_signature_invalid")
    envelope = build_promotion_envelope(statement, signature=signature)
    envelope_bytes = canonical_json(envelope).encode("utf-8")
    write_private_file(
        arguments.promotion_authorization_out,
        envelope_bytes,
        maximum_bytes=AUTHORIZATION_MAXIMUM_BYTES,
    )
    return {
...
def _verify(arguments: argparse.Namespace) -> dict[str, Any]:
    document = _read_stable_file(arguments.promotion_authorization)
    parse_promotion_authorization_envelope(document)
    result = verify_promotion_with_trust_store(_required_url(VERIFIER_URL_ENV), document)
...
def _admit(arguments: argparse.Namespace) -> dict[str, Any]:
    document = _read_stable_file(arguments.promotion_authorization)
    parse_promotion_authorization_envelope(document)
    result = verify_and_admit_promotion_authorization(_required_url(VERIFIER_URL_ENV), document)
    if result["result"] == "rejected":
        raise Phase5C4PromotionAuthorizationControlError(str(result["reason"]))
    return {"command": "admit", **result}


def _bootstrap(arguments: argparse.Namespace) -> dict[str, Any]:
    public_key = _read_stable_file(arguments.public_key_der, maximum_bytes=1024)
    public_key_der_and_id(public_key)
    result = bootstrap_promotion_authorization_key(
...
def _revoke_key(arguments: argparse.Namespace) -> dict[str, Any]:
    result = revoke_promotion_authorization_key(
        _required_url(MIGRATOR_URL_ENV),
        key_id=arguments.key_id,
...
def _revoke(arguments: argparse.Namespace) -> dict[str, Any]:
    result = revoke_promotion_authorization(
        _required_url(MIGRATOR_URL_ENV),
...
def _exit_code(reason: str) -> int:
    if reason in {
        "promotion_authorization_key_unknown",
        "promotion_authorization_key_untrusted",
        "promotion_authorization_revoked",
        "promotion_authorization_unauthorized",
    }:
        return 4
    if reason in {
        "promotion_authorization_binding_stale",
        "promotion_authorization_conflict",
        "promotion_authorization_key_conflict",
    }:
        return 5
    if reason in {
        "promotion_authorization_database_unavailable",
        "promotion_authorization_retry",
    }:
        return 6
    if reason == "promotion_authorization_internal_failure":
        return 9
    return 3
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        reason = getattr(
            exc,
            "reason_code",
            "promotion_authorization_file_invalid",
        )
        sys.stderr.write(
            canonical_json(
                {
        ...
        sys.stderr.write(
            canonical_json(
                {
                    "command": arguments.command,
                    "reason": "promotion_authorization_internal_failure",
                    "result": "rejected",
                }
            )
            + "\n"
