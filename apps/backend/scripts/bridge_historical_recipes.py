...
from app.operators.historical_recipe_bridge import bridge_legacy_recipes
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preserve canonical pre-0004 Recipe tables on an offline conversion clone."
    )
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit("Unable to bridge the explicitly configured conversion clone") from None

    output = result.to_json() if args.format == "json" else result.to_human()
