"""Manage public Phase 5C4.8 cutback-authorization material.
...
from app.operators.phase5c4_cutback_authorization_control import (
    ...
    read_cutback_authorization_status,
    revoke_cutback_authorization,
    revoke_cutback_authorization_key,
    verify_and_admit_cutback_authorization,
    verify_cutback_with_trust_store,
)


VERIFIER_URL_ENV = "NUTRITION_PHASE5C4_CUTBACK_AUTHORIZATION_VERIFIER_DATABASE_URL"
...
def _read_stable_file(
    ...
    *,
    ...
    try:
        ...
        raise Phase5C4AuthorizationError("cutback_authorization_file_invalid") from None
    try:
        ...
        if (
            ...
            raise Phase5C4AuthorizationError("cutback_authorization_file_invalid")
        chunks: list[bytes] = []
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Manage public Phase 5C4.8 cutback authorization material. "
            "This command cannot sign, switch routes, restore source writes, "
            "or load a private key."
        )
    )
    ...
    verify.add_argument("--cutback-authorization", type=Path, required=True)

    admit = commands.add_parser("admit")
    admit.add_argument("--cutback-authorization", type=Path, required=True)
...
def _parse_time(value: str) -> datetime:
    try:
        ...
        raise Phase5C4AuthorizationError("cutback_authorization_time_invalid") from None
    if canonical_timestamp(parsed) != value:
        raise Phase5C4AuthorizationError("cutback_authorization_time_invalid")
    return parsed
...
def _export(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        raise Phase5C4AuthorizationError("cutback_authorization_invalid") from None
...
def _assemble(arguments: argparse.Namespace) -> dict[str, Any]:
    statement = parse_cutback_signed_statement(_read_stable_file(arguments.statement))
    signature = _read_stable_file(
        arguments.signature_file,
        maximum_bytes=64,
    )
    if len(signature) != 64:
        raise Phase5C4AuthorizationError("cutback_authorization_signature_invalid")
    envelope = build_cutback_envelope(statement, signature=signature)
    document = canonical_json(envelope).encode("utf-8")
    write_private_file(
        arguments.cutback_authorization_out,
        document,
        maximum_bytes=AUTHORIZATION_MAXIMUM_BYTES,
    )
    return {
...
def _verify(arguments: argparse.Namespace) -> dict[str, Any]:
    document = _read_stable_file(arguments.cutback_authorization)
    parse_cutback_authorization_envelope(document)
    result = verify_cutback_with_trust_store(
...
def _admit(arguments: argparse.Namespace) -> dict[str, Any]:
    document = _read_stable_file(arguments.cutback_authorization)
    parse_cutback_authorization_envelope(document)
    result = verify_and_admit_cutback_authorization(
        _required_url(VERIFIER_URL_ENV),
        document,
    )
    if result["result"] == "rejected":
        raise Phase5C4CutbackAuthorizationControlError(str(result["reason"]))
...
def _bootstrap(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    result = bootstrap_cutback_authorization_key(
...
def _revoke_key(arguments: argparse.Namespace) -> dict[str, Any]:
    result = revoke_cutback_authorization_key(
        _required_url(MIGRATOR_URL_ENV),
        key_id=arguments.key_id,
...
def _revoke(arguments: argparse.Namespace) -> dict[str, Any]:
    result = revoke_cutback_authorization(
        _required_url(MIGRATOR_URL_ENV),
...
def _status(arguments: argparse.Namespace) -> dict[str, Any]:
    row = read_cutback_authorization_status(
        _required_url(AUDIT_URL_ENV),
        arguments.authorization_id,
    )
    if row is None:
        raise Phase5C4CutbackAuthorizationControlError("cutback_authorization_unknown")
    return {"command": "status", "result": "accepted", **row}


def _exit_code(reason: str) -> int:
    if reason in {
        "cutback_authorization_key_unknown",
        "cutback_authorization_key_untrusted",
        "cutback_authorization_revoked",
        "cutback_authorization_unauthorized",
        "cutback_authorization_unknown",
    }:
        return 4
    if reason in {
        "cutback_authorization_binding_stale",
        "cutback_authorization_conflict",
    ...
    if reason in {
        ...
        return 6
    if reason == "cutback_authorization_internal_failure":
        return 9
    return 3
    ...
    try:
        ...
        sys.stderr.write(
            canonical_json(
                {
                    "command": arguments.command,
                    "reason": "cutback_authorization_internal_failure",
                    "result": "rejected",
                }
            )
            + "\n"
