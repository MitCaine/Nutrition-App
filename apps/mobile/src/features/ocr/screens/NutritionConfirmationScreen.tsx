...
import { confirmationPayload, confirmationValidationIssue, omitReview, updateReview } from "../confirmation/confirmationModel";
...
export function NutritionConfirmationScreen({ initialDraft, onCancel, onCreated }: {
  ...
  const [validationFieldKey, setValidationFieldKey] = useState<string | null>(null);
  ...
  const submit = async () => {
    ...
    const validation = confirmationValidationIssue(draft);
    if (validation) {
      validationFocusKeyRef.current = confirmationValidationFocusKey(validation.fieldKey);
      setValidationFieldKey(validation.fieldKey);
    ...
    submittingRef.current = true; setSubmitting(true); setValidationFieldKey(null); setError(null);
    try {
  ...
  return <View style={styles.screen}>
    <KeyboardSafeScrollView ref={scrollRef} contentContainerStyle={styles.content}>{(focusProps) => <>
      ...
      <LabeledField {...focusProps("ocr.name")} label="Food name" validationTarget="ocr.name" required disabled={submitting} invalid={validationFieldKey === "food.name"} error={validationFieldKey === "food.name" ? error : null} value={draft.name} onChangeText={(name) => setDraft({ ...draft, name })} placeholder="Food name" placeholderTextColor={theme.colors.placeholder} inputStyle={styles.input}/>
      ...
      {fields.map((field) => <View key={field.fieldKey} style={[styles.card, field.decision === "unresolved" && styles.flagged, field.decision === "omitted" && styles.omitted]}>
        ...
        <LabeledField {...focusProps(`ocr.field.${field.fieldKey}`)} label={`${field.label} amount`} validationTarget={`ocr.field.${field.fieldKey}`} disabled={submitting || field.decision === "omitted"} invalid={validationFieldKey === field.fieldKey} error={validationFieldKey === field.fieldKey ? error : null} value={field.confirmedValue} onChangeText={(value) => replaceField(updateReview(field, value))} keyboardType="decimal-pad" inputStyle={styles.input}/>
        <View style={styles.actions}>{showsUseValueAction(field) ? <AccessiblePressable accessibilityLabel={`Use ${field.label} value`} disabled={submitting} onPress={() => { const value = field.decision === "omitted" ? field.suggestedValue ?? "" : field.confirmedValue; replaceField(updateReview(field, value, value === (field.suggestedValue ?? "") ? "accepted" : "edited")); }}><Text style={styles.link}>Use value</Text></AccessiblePressable> : null}<AccessiblePressable accessibilityLabel={`Omit ${field.label}`} disabled={submitting || field.nutrientId === "calories"} onPress={() => replaceField(omitReview(field))}><Text style={styles.link}>Omit</Text></AccessiblePressable></View>
      ...
      {error && validationFieldKey === null ? <Text ref={errorRef} accessibilityLiveRegion="none" accessibilityRole="alert" style={styles.error}>{error}</Text> : null}
