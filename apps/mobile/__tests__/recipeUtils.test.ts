...
test("serving multipliers distinguish recipe quantity from serving size", () => {
  expect(formatServingMultiplier("1", "2 slices")).toBe("2 slices");
  expect(formatServingMultiplier("3", "2 slices")).toBe("3 × 2 slices");
});

test("custom serving definitions derive their normal display name from quantity and unit", () => {
...
test("recipe validation requires name, positive amounts, valid serving references, and positive yield", () => {
  expect(validateRecipeDraft(emptyRecipeDraft())).toBe("Recipe name is required.");
  const missingServing = { ...ingredientForFood(food), servingDefinitionId: null };
  expect(validateRecipeDraft({ ...emptyRecipeDraft(), name: "Missing", ingredients: [missingServing] })).toBe("Serving ingredients need a selected serving.");
  const invalidAmount = { ...ingredientForFood(food), amountQuantity: "0" };
  expect(validateRecipeDraft({ ...emptyRecipeDraft(), name: "Invalid", ingredients: [invalidAmount] })).toBe("Ingredient amounts must be greater than zero.");
  const missingDefinition = { ...ingredientForFood(food), servingDefinitionId: "deleted-serving" };
  expect(validateRecipeDraft({ ...emptyRecipeDraft(), name: "Missing definition", ingredients: [missingDefinition] })).toBe("Selected serving is no longer available for an ingredient.");
  expect(validateRecipeDraft({ ...emptyRecipeDraft(), name: "Yield", servingCountYield: "0" })).toBe("Serving yield must be greater than zero.");
  expect(validateRecipeDraft({ ...emptyRecipeDraft(), name: "Valid", servingCountYield: "4" })).toBeNull();
});

test("publish eligibility accepts serving count or cooked weight", () => {
  expect(canPublishRecipe({ servingCountYield: "4", finalCookedWeightGrams: "" })).toBe(true);
  expect(canPublishRecipe({ servingCountYield: "", finalCookedWeightGrams: "1000" })).toBe(true);
  expect(canPublishRecipe({ servingCountYield: "", finalCookedWeightGrams: "" })).toBe(false);
});

test("recipe total display preserves unknown nutrient coverage", () => {
  expect(formatRecipeTotal({ amount: "10", unit: "g", known_ingredient_count: 1, unknown_ingredient_count: 0 })).toBe("10 g");
  expect(formatRecipeTotal({ amount: "10", unit: "g", known_ingredient_count: 1, unknown_ingredient_count: 2 })).toBe("10 g + unknown from 2 items");
  expect(formatRecipeTotal({ amount: null, unit: "g", known_ingredient_count: 0, unknown_ingredient_count: 1 })).toBe("Unknown from 1 item");
});
