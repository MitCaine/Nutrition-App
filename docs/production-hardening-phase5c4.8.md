...
The implemented predecessor surfaces remain fixed:

- `manage_phase5c4_promotion_authorization.py` handles promotion authorization
  material;
- `manage_phase5c4_execution_authorization.py` handles schema-execution
  authorization material;
- `manage_phase5c4_control.py` exposes fixed control requests, observations,
  reconciliation, status, evidence export, and outbox delivery;
- `manage_phase5c4_target_activation.py` performs only target migration,
  read-only target inspection, target opening, and emergency close from durable
  control actions; and
- `manage_phase5c4_recovery.py` executes, validates, and audits an explicitly
  identified disposable restore.

The Phase 5C4.8 command and API boundary contains only these bounded
capabilities:

- a signerless cutback-authorization CLI with the established
  export/assemble/verify/admit pattern and migrator-only public-key and
  revocation operations;
- fixed security-definer APIs for cutback intent, route observation,
  source-restoration intent and observation, exact reconciliation, terminal
  completion, and read-only recovery status;
- a source-local CLI that can inspect the source or restore source runtime
  writes only from one exact durable cutback action; and
- a read-only cumulative snapshot/PITR qualification CLI that consumes
  authoritative observations and existing immutable recovery evidence.
